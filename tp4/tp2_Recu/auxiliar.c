...
static int getInt(int* pResultado)
{
    ...
    if(	pResultado != NULL &&
		...
		aux_verificarNumerica(auxString,sizeof(auxString)) == 0)
	{
 ...
 *
 */
int getStringNum(char* cadena, int longitud)
{
	int retorno = -1;
	char auxString[64];
	long aux;
...
int aux_getCadena(char* pResultado, int longitud,char* mensaje, char* mensajeError, int reintentos)
{
	char bufferString[4096];
	int retorno = -1;
	while(reintentos>=0)
	{
		reintentos--;
		printf("%s",mensaje);
		if(getString(bufferString,sizeof(bufferString)) == 0 && strnlen(bufferString,sizeof(bufferString)) < longitud && esNombre(bufferString,51) == 1)
		{
			strncpy(pResultado,bufferString,longitud);
			retorno = 0;
			break;
